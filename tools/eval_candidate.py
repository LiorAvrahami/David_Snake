...
class Replay:
    def __init__(self, gid, row, traj):
        ...
        for j in range(1, self.n + 1):
            ...
            a = rel(v)
            ln = math.hypot(*v)
            self.ev.append((0.0 if (abs(a) < 30 or ln < 2) else angle_score(a), ln, best))
        vf = p[-1]
        af = rel(vf)
        self.lnf = math.hypot(*vf)
        self.aSf = 0.0 if (abs(af) < 30 or self.lnf < 2) else angle_score(af)
        self.peakf = self.ev[-1][2]
    ...
    def simulate(self, new):
        ...
        lS = lambda l: l / (l + K)
        fire_ms = None
        for j, (a, ln, pk) in enumerate(self.ev):
